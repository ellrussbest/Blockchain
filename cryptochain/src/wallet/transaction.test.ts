...
describe("Transaction", () => {
	...
	describe("rewardTransaction()", () => {
		let _rewardTransaction: transaction.BlockRewardTx;
		let minerWallet: Wallet;

		beforeEach(() => {
			minerWallet = new Wallet();
			_rewardTransaction = rewardTransaction({
				minerWallet,
			});
		});

		it("creates a transaction with the reward input", () => {
			expect(_rewardTransaction.input).toEqual(REWARD_INPUT);
		});

		it("creates one transaction for the miner with the `MINING_REWARD`", () => {
			expect(_rewardTransaction.outputMap[minerWallet.publicKey]).toEqual(
				MINING_REWARD,
			);
		});
	});
